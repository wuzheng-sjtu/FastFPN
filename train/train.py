...
def solve(global_step):
    ...
    # learning reate
    lr = _configure_learning_rate(2950, global_step)
...
def train():
    ...
    ## data
    image, ih, iw, gt_boxes, num_instances, img_id = \
    ...
    enqueue_op = data_queue.enqueue((image, ih, iw, gt_boxes, num_instances, img_id))
    data_queue_runner = tf.train.QueueRunner(data_queue, [enqueue_op] * 4)
    tf.add_to_collection(tf.GraphKeys.QUEUE_RUNNERS, data_queue_runner)
    (image, ih, iw, gt_boxes, num_instances, img_id) =  data_queue.dequeue()
    ...
    for step in range(FLAGS.max_iters):
        ...
        if step % 1 == 0: 
            print ( """iter %d: image-id:%07d, time:%.3f(sec), regular_loss: %.6f, """
                    """total-loss %.4f(%.4f, %.4f, %.6f, %.4f), """
                    """instances: %d, """
                    """batch:(%d|%d, %d|%d)""" 
                   % (step, img_id_str, duration_time, reg_lossnp, 
                      tot_loss, rpn_box_loss, rpn_cls_loss, refined_box_loss, refined_cls_loss,
                      gt_boxesnp.shape[0], 
                      rpn_batch_pos, rpn_batch, refine_batch_pos, refine_batch))
